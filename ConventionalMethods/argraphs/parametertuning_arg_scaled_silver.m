...
if (plot_results)
    ...
    F = getframe(FigH);imwrite(F.cdata, strcat(foldername , '\F1.png'), 'png');
    close(FigH)
    ...
    F = getframe(FigH);imwrite(F.cdata, strcat(foldername , '\AJI.png'), 'png');
    close(FigH)
    ...
    title('AJI + F1');
    F = getframe(FigH);imwrite(F.cdata, strcat(foldername , '\combined.png'), 'png');
    close(FigH)
end

% Save results

save(strcat(foldername , '\results.mat', 'results'));
save(strcat(foldername , '\combined_results.mat', 'combined_results'));
save(strcat(foldername , '\results_chosen.mat', 'erg'));
save(strcat(foldername , '\position_max.mat', 'max_pos'));
