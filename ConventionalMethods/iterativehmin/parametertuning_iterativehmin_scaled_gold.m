...
results = [];
tarea = [3,5,7,10,12,15,20];
dSize = [5,7,10,12,15,20,25];
gAngle = [1,3,5,7,10,15,20];
...
if (plot_results)
    ...
    F = getframe(FigH);imwrite(F.cdata, foldername + '\F1.png', 'png');
    close(FigH)
    ...
    F = getframe(FigH);imwrite(F.cdata, foldername + '\AJI.png', 'png');
    close(FigH)
    FigH = figure('Position', get(0, 'Screensize'))
    hold on; 
    plot([max_pos max_pos],[-1 1.2],'--k','LineWidth',2);ylim([-1 1.2]);
    plot([0 size(results,1)],[combined_results(max_pos) combined_results(max_pos)],'--k','LineWidth',2);ylim([-1 1.2]);xlim([0 length(results(:,7))]);
    plot(1:length(results(:,7)), combined_results(:),'ko','LineWidth',1);
    title("AJI + F1");
    F = getframe(FigH);imwrite(F.cdata, foldername + '\combined.png', 'png');
    close(FigH)
end
